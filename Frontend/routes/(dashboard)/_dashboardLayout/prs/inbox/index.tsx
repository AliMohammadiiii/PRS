...
import { extractErrorMessage, getPrsStatusColors, hasRole } from 'src/shared/utils/prsUtils';
...
export const Route = createFileRoute('/(dashboard)/_dashboardLayout/prs/inbox/')({
  beforeLoad: async () => {
    try {
      // Ensure only admins / approvers can access the inbox
      const user = await authApi.getMe();

      const isAdmin = user.is_admin || hasRole(user, 'ADMIN');
      const isApprover = hasRole(user, 'APPROVER');

      if (!isAdmin && !isApprover) {
        // Non-approver requesters should be routed back to their own requests
        throw redirect({ to: '/prs/my-requests' });
      }
    } catch {
      // If we cannot load the user (e.g. token expired), send them to login
      throw redirect({ to: '/login' });
    }
  },
  component: ApproverInboxPage,
});
...
function ApproverInboxPage() {
  ...
  const loadRequests = useCallback(async () => {
    try {
      setIsLoading(true);
      const data = await prsApi.getMyApprovals(filters);
      setRequests(data);
    } catch (err: any) {
      ...
      logger.error('Error loading approvals:', err);
    } finally {
      setIsLoading(false);
    }
  }, [filters]);
  ...
  const handleFilterChange = (key: keyof PrsInboxFilters, value: any) => {
    setFilters(prev => ({
      ...prev,
      [key]: value || undefined,
    }));
  ...
  if (isLoading && requests.length === 0) {
    return (
      <>
        <PageHeader
          title="صندوق ورودی تاییدکنندگان"
          breadcrumb={['درخواست‌های خرید', 'صندوق ورودی']}
        />
        <Box
          sx={{
            bgcolor: 'white',
        ...
        {requests.length === 0 && !isLoading ? (
          <Box sx={{ textAlign: 'center', py: 8 }}>
            <Typography variant="body1" color="text.secondary">
              در حال حاضر درخواستی برای بررسی شما وجود ندارد
            </Typography>
          </Box>
        ) : (
          <DataGrid
